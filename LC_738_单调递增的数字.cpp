...
public:
	...
	//第二次做，还是没有做出来
	int monotoneIncreasingDigits1(int n) {  //这个思路有问题，输入1234，最后返回999，是因为position初始化为了0
	...
	int monotoneIncreasingDigits2(int n) {  //然后把position初始化成了s.size()，但是在面对668841这样的数时，又会返回599999，正确答案应该是667999
		string s = to_string(n);
		int position = s.size();
		for (int i = 0; i < s.size() - 1; i++) {
			if (s[i] >= s[i + 1]) {
	...
	int monotoneIncreasingDigits3(int n) {  //这个思路才是正确的
		string s = to_string(n);
		int position = s.size();
