...
public:
    ...
    // 双指针，从两端向中间，对于某个位置，如果它左边的最大高度比右边最大高度小的话，那么就用左边的最大高度可以求出水位高度，因为右边的最大高度只会增高，该处的水位始终受左边的限制；
    // 对于右边来说同理，如果它右边的最大高度小于左边的最大高度，那么就用右边的最大高度求出左边的最大高度。
    int trap7(vector<int> &height) {
        int n = height.size();
        int i = 0, j = n - 1;
        ...
        while (i <= j) {
            if (pre_max < suf_max) {
                if (pre_max < height[i])
                    ...
                    res += pre_max - height[i];
                i++;
            } else {
                if (suf_max < height[j])
                    suf_max = height[j];
                else
                    res += suf_max - height[j];
                j--;
            }
        }
        return res;
